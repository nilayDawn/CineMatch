...
# FASTAPI APP
# =========================
app = FastAPI(title="TMDB Movie Recommender API", version="1.0")
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],
    allow_methods=["*"],
...
DF_PATH = os.path.join(BASE_DIR, "movies.pkl")
INDICES_PATH = os.path.join(BASE_DIR, "indices.pkl")
TF_IDF_MATRIX_PATH = os.path.join(BASE_DIR, "tfidf_matrix.pkl")
TF_IDF_PATH = os.path.join(BASE_DIR, "tfidf.pkl")
...
def get_local_idx_by_title(title: str) -> int:
    global TITLE_TO_IDX
    if TITLE_TO_IDX is None: # pyright: ignore[reportUnboundVariable]
        raise HTTPException(status_code=500, detail="TF-IDF index map not initialized")
    key = _norm_title(title)
    if key in TITLE_TO_IDX: # type: ignore
        return int(TITLE_TO_IDX[key]) # pyright: ignore[reportUnboundVariable]
    raise HTTPException(
...
def load_pickles():
    ...
    with open(TF_IDF_MATRIX_PATH, "rb") as f:
